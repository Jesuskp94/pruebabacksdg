 ...
 router.post('/', (req, res) => {
  ...
  if(mensaje) {
    if(mandarMensaje(mensaje)) {
      res.json({respuesta: "Todos los mensajes se han mandado con éxito"});
    } else {
      res.status(400).json({respuesta: "El mensaje no se ha podido mandar, no hay usuarios subscritos"});
    }
  } else {
    res.status(400).json({respuesta: "No hay texto en el mensaje."});
  }
})
...
function mandarMensaje(mensaje)
{
  ...
  if(usuariosSubscritos.length > 0) {
    for (let contador = 0; contador < usuariosSubscritos.length; contador++) {
      console.log({respuesta: 'mensaje enviado a ' + usuariosSubscritos[contador].nickname});
    }
    return true;
  }
  return false;
